...
static gatt_ctrl_t trans_gatt_control_block = {
    //public
    .mtu_size = ATT_LOCAL_MTU_SIZE,// 设置本地MTU,MTU决定了单次可以传输的最大数据包长度
    .cbuffer_size = ATT_SEND_CBUF_SIZE,// 设置发送缓冲区的大小
    .multi_dev_flag	= 0, // 多设备连接标志，设置为0表示不支持同时连接多个设备
...
static void trans_uart_rx_to_ble(u8 *packet, u32 size)
{
    log_info("UART RX: size=%d, conn_handle=%04x\n", size, trans_con_handle);
    
    if (!trans_con_handle) {
        log_info("No connection handle\n");
        return;
    }
    
    if (!ble_comm_att_check_send(trans_con_handle, size)) {
        log_info("ATT send check failed\n");
        return;
    }
    
    u8 ccc_value = ble_gatt_server_characteristic_ccc_get(trans_con_handle, ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE);
    log_info("CCC value: %02x\n", ccc_value);
    
    
    if (ccc_value) {
        ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_ae02_01_VALUE_HANDLE, packet, size, ATT_OP_AUTO_READ_CCC);
    } else {
        log_info("Client not enabled notifications\n");
    }
}
